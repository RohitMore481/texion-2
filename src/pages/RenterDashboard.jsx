...
export default function RenterDashboard() {
  ...
  return (
    <div style={{ flex: 1, display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
      <main className="container" style={{ 
        ...
        <div style={{ 
          ...
          <div className="glass-panel" style={{ height: '100%', position: 'relative' }}>
            ...
            <div style={{ position: 'absolute', top: 16, right: 16, zIndex: 500, display: 'flex', gap: '0.5rem', background: 'var(--bg-surface-elevated)', padding: '0.5rem', borderRadius: 'var(--radius-full)', backdropFilter: 'blur(8px)' }}>
              <button onClick={() => setViewMode('split')} style={{ padding: '0.5rem', borderRadius: '50%', background: viewMode === 'split' ? 'var(--accent-primary)' : 'transparent', color: viewMode === 'split' ? 'white' : 'var(--text-primary)' }}><LayoutGrid size={18} /></button>
              <button onClick={() => setViewMode('map')} style={{ padding: '0.5rem', borderRadius: '50%', background: viewMode === 'map' ? 'var(--accent-primary)' : 'transparent', color: viewMode === 'map' ? 'white' : 'var(--text-primary)' }}><Map size={18} /></button>
            </div>
          </div>
        </div>
        ...
        <div style={{ 
          ...
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '1.5rem' }}>
            <h2 style={{ fontSize: '1.25rem', margin: 0 }}>{properties.length} Properties Found</h2>
            {viewMode === 'list' && (
      ...
      <ComparisonView />
      {contactProperty && <BrokerContact property={contactProperty} onClose={() => setContactProperty(null)} />}
    </div>
  );
}
